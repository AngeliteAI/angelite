...
public func createEmptyVolume(size_x: UInt32, size_y: UInt32, size_z: UInt32) -> UnsafeMutableRawPointer? {
    ...
    let voxelGrid = VoxelGrid(data: Palette(single: 0, count: Int(size_x * size_y * size_z)), size: IVec3(x: Int32(size_x), y: Int32(size_y), z: Int32(size_z)), position: v3Zero(), rotation: qId())
    ...
    print("createEmptyVolume (Swift): size = (\(size_x), \(size_y), \(size_z)), id = \(volumePointer.pointee.id)")
    return volumeRawPointer
}

@_cdecl("createVolumeFromSDF")
public func createVolumeFromSDF(sdf: UnsafeMutableRawPointer?, brush: UnsafeRawPointer?, position: UnsafeRawPointer, size: UnsafeRawPointer) -> UnsafeMutableRawPointer? {
...
@_cdecl("unionVolume")
public func unionVolume(vol: UnsafeMutableRawPointer?, transform: UnsafeRawPointer?, brush: UnsafeRawPointer?, position: UnsafeRawPointer) -> UnsafeMutableRawPointer? {
    print("unionVolume (Swift stub)")
    return nil
}

@_cdecl("subtractVolume")
public func subtractVolume(vol: UnsafeMutableRawPointer?, transform: UnsafeRawPointer?, position: UnsafeRawPointer) -> UnsafeMutableRawPointer? {
    print("subtractVolume (Swift stub)")
    return nil
}

@_cdecl("replaceVolume")
public func replaceVolume(vol: UnsafeMutableRawPointer?, transform: UnsafeRawPointer?, brush: UnsafeRawPointer?, position: UnsafeRawPointer) -> UnsafeMutableRawPointer? {
    print("replaceVolume (Swift stub)")
    return nil
}

@_cdecl("paintVolumeRegion")
public func paintVolumeRegion(vol: UnsafeMutableRawPointer?, transform: UnsafeRawPointer?, brush: UnsafeRawPointer?, position: UnsafeRawPointer) -> UnsafeMutableRawPointer? {
    print("paintVolumeRegion (Swift stub)")
    return nil
}

@_cdecl("mergeVolumes")
public func mergeVolumes(volumes: UnsafeMutableRawPointer?, count: Int32) -> UnsafeMutableRawPointer? {
    print("mergeVolumes (Swift stub)")
    return nil
}

@_cdecl("extractRegion")
public func extractRegion(vol: UnsafeMutableRawPointer?, min_x: Int32, min_y: Int32, min_z: Int32, max_x: Int32, max_y: Int32, max_z: Int32) -> UnsafeMutableRawPointer? {
...
public func placeStructure(world: UnsafeMutableRawPointer?, structure: UnsafeMutableRawPointer?, position: UnsafeRawPointer, rotation: UInt8) -> UnsafeMutableRawPointer? {
...
public func getVoxel(
    ...
    let size = voxelGrid.size          // IVec3(Int32)
    let sx = Int(size.x)
    let sy = Int(size.y)
    ...
    guard sx > 0, sy > 0, sz > 0, voxelGrid.data != nil else {
         print("getVoxel (Swift Error): Invalid grid dimensions or data pointer for volume id \(volumeId). (\(sx)x\(sy)x\(sz))")
         typedOutBlocks.initialize(repeating: 0, count: count)
         return
    }
    for i in 0..<count {
        let pos = typedPositions[i]
        let x = Int(pos.x)
        let y = Int(pos.y)
        let z = Int(pos.z)
        if x < 0 || x >= sx || y < 0 || y >= sy || z < 0 || z >= sz {
            ...
            typedOutBlocks[i] = voxelGrid.data[index]
        }
    }
...
@_cdecl("setVoxel")
public func setVoxel(vol: UnsafeMutableRawPointer?, positions: UnsafeRawPointer?, blocks: UnsafeRawPointer?, count: Int) {
    print("setVoxel (Swift stub)")
}

@_cdecl("getVolumeSize")
public func getVolumeSize(vol: UnsafeMutableRawPointer?, size_ptr: UnsafeMutablePointer<SIMD3<UInt32>>?) {
    print("getVolumeSize (Swift stub)")
}

@_cdecl("getVolumePosition")
public func getVolumePosition(vol: UnsafeMutableRawPointer?, position_ptr: UnsafeMutablePointer<SIMD3<Int32>>?) {
    print("getVolumePosition (Swift stub)")
}

@_cdecl("moveVolume")
public func moveVolume(vol: UnsafeMutableRawPointer?, x: Int32, y: Int32, z: Int32) -> UnsafeMutableRawPointer? {
