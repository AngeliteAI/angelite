...
const PhysicsEngine = struct {
    ...
    // Compute pipelines
    broad_phase_pipeline: ?vk.Pipeline,
    narrow_phase_pipeline: ?vk.Pipeline,
    ...
    pub fn deinit(self: *PhysicsEngine) void {
        ...
        if (self.device) |device| {
            _ = device;
            // TODO: Destroy pipelines, buffers, etc.
        }
    ...
    fn createComputePipelines(self: *PhysicsEngine) !void {
        ...
        std.debug.print("Physics command buffer and descriptors allocated successfully\n", .{});
        ...
        var set_layout: vk.DescriptorSetLayout = undefined;
        _ = dispatch.vkCreateDescriptorSetLayout.?(device, &layout_info, null, &set_layout);
        self.descriptor_set_layout = set_layout;

        // Create pipeline layout with push constants for delta time and other parameters
        // Integration shader needs: float delta_time, vec3 gravity, uint body_count = 32 bytes
        // Broadphase shader needs: uint pair_count = 4 bytes (padded to 8)
        // Use the largest size needed
        const push_constant_range = vk.PushConstantRange{
            .stage_flags = .{ .compute_bit = true },
            .offset = 0,
            .size = 32, // Max size needed by integration shader
        };

        const set_layouts = [_]vk.DescriptorSetLayout{set_layout};
        const push_constant_ranges = [_]vk.PushConstantRange{push_constant_range};
        ...
        _ = dispatch.vkCreatePipelineLayout.?(device, &pipeline_layout_info, null, &pipeline_layout);
        self.compute_pipeline_layout = pipeline_layout;

        // Create shader modules using loadShaderModule
        const integration_module = try self.loadShaderModule("physics_integration.comp.spirv");
        defer dispatch.vkDestroyShaderModule.?(device, integration_module, null);

        const broadphase_module = try self.loadShaderModule("physics_broadphase.comp.spirv");
        defer dispatch.vkDestroyShaderModule.?(device, broadphase_module, null);

        const narrowphase_module = try self.loadShaderModule("physics_narrowphase.comp.spirv");
        defer dispatch.vkDestroyShaderModule.?(device, narrowphase_module, null);

        const resolve_module = try self.loadShaderModule("physics_resolve.comp.spirv");
        defer dispatch.vkDestroyShaderModule.?(device, resolve_module, null);

        // Create pipelines
        const pipeline_infos = [_]vk.ComputePipelineCreateInfo{
            .{ // Integration
                .flags = .{},
                .stage = .{
                    .flags = .{},
        ...
        var pipelines: [4]vk.Pipeline = undefined;
        _ = dispatch.vkCreateComputePipelines.?(device, .null_handle, pipeline_infos.len, &pipeline_infos, null, &pipelines);

        self.integration_pipeline = pipelines[0];
        self.broad_phase_pipeline = pipelines[1];
        self.narrow_phase_pipeline = pipelines[2];
        self.resolve_pipeline = pipelines[3];
    ...
    fn allocateCommandBuffer(self: *PhysicsEngine) !void {
        ...
        const pool_sizes = [_]vk.DescriptorPoolSize{
            .{
                .type = .storage_buffer,
                .descriptor_count = 3, // Updated to include contact buffer
            },
        };
        ...
        _ = dispatch.vkCreateDescriptorPool.?(device, &pool_info, null, &desc_pool);
        self.descriptor_pool = desc_pool;

        // Allocate descriptor set
        const set_layout = self.descriptor_set_layout orelse return error.NoDescriptorSetLayout;
        const set_layouts_array = [_]vk.DescriptorSetLayout{set_layout};
        const set_alloc_info = vk.DescriptorSetAllocateInfo{
            .descriptor_pool = desc_pool,
            .descriptor_set_count = 1,
            .p_set_layouts = &set_layouts_array,
        };

        var desc_set: vk.DescriptorSet = undefined;
        _ = dispatch.vkAllocateDescriptorSets.?(device, &set_alloc_info, @as([*]vk.DescriptorSet, @ptrCast(&desc_set)));
        self.descriptor_set = desc_set;

        // Update descriptor set with our buffers
        const rigidbody_buffer_info = vk.DescriptorBufferInfo{
            .buffer = self.rigidbody_buffer orelse return error.NoRigidbodyBuffer,
            .offset = 0,
            .range = vk.WHOLE_SIZE,
        };

        const collision_buffer_info = vk.DescriptorBufferInfo{
            .buffer = self.collision_pairs_buffer orelse return error.NoCollisionBuffer,
            .offset = 0,
            .range = vk.WHOLE_SIZE,
        };

        const contact_buffer_info = vk.DescriptorBufferInfo{
            .buffer = self.contact_buffer orelse return error.NoContactBuffer,
            .offset = 0,
            .range = vk.WHOLE_SIZE,
        };
        ...
        const write_sets = [_]vk.WriteDescriptorSet{
            .{
                ...
                .p_buffer_info = @ptrCast(&rigidbody_buffer_info),
            ...
            .{
                ...
                .descriptor_type = .storage_buffer,
                .p_image_info = undefined,
                .p_buffer_info = @ptrCast(&collision_buffer_info),
                .p_texel_buffer_view = undefined,
            },
            .{
                .dst_set = desc_set,
                ...
                .descriptor_type = .storage_buffer,
                .p_image_info = undefined,
                .p_buffer_info = @ptrCast(&contact_buffer_info),
                .p_texel_buffer_view = undefined,
            },
        };

        dispatch.vkUpdateDescriptorSets.?(device, write_sets.len, &write_sets, 0, null);
    }
    ...
    fn stepGPU(self: *PhysicsEngine, dt: f32) void {
        // In a full GPU implementation, we would:
        // 1. Upload rigidbody data to GPU buffer
        // 2. Clear collision pair count
        // 3. Dispatch broad phase (find potential collisions)
        // 4. Dispatch narrow phase (test actual collisions)
        // 5. Dispatch collision resolution
        // 6. Dispatch integration (update positions)
        // 7. Read back rigidbody data
