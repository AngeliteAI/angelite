...
impl GpuWorldGenPipeline {
    pub fn new(gfx: Arc<dyn Gfx + Send + Sync>) -> Self {
        ...
        let (coordinator, main_sender) = MainThreadCoordinator::new();
...
/// Simple single-threaded async runtime for blocking on futures
...
impl SingleThreadedRuntime {
    ...
    where
        F: std::future::Future,
    {
        // Since the GPU world generator is currently synchronous,
        // we can just use a simple blocking implementation
        use std::pin::Pin;
        ...
        loop {
            match future.as_mut().poll(&mut cx) {
                ...
                Poll::Pending => {
                    // In a real runtime, we would yield to other tasks here
