...
impl VoxelRendererBridge {
    ...
    pub async fn add_chunk(&mut self, chunk: CompressedChunk) -> Result<(), String> {
        ...
        let (vertices, _indices) = self.generate_greedy_mesh_for_chunk(&chunk).await?;
        ...
        // Update mesh data
        println!("Updating mesh with {} vertices", positions.len());
    ...
    async fn generate_greedy_mesh_for_chunk(
        ...
        println!("Decompressing chunk with palette size: {}, bits_per_index: {}", 
            chunk.palette.len(), chunk.indices.bits_per_index);
        ...
        println!("Decompressed chunk has {} non-air voxels out of {}", non_air_count, decompressed.len());
        
        // Debug: Print first few voxels
        if decompressed.len() > 0 {
            println!("First 10 voxels: {:?}", &decompressed[..10.min(decompressed.len())]);
        }
        
        // Use the existing greedy mesh generation
        let renderer = VertexPoolBatchRenderer::new(self.gfx.clone());
        let result = renderer.generate_greedy_mesh(&decompressed, chunk_size);
        
        match &result {
            Ok((vertices, indices)) => {
                println!("Mesh generation successful: {} vertices, {} indices", vertices.len(), indices.len());
                if vertices.len() > 0 {
                    println!("First vertex: pos={:?}, size={:?}, normal={}, color={:?}", 
                        vertices[0].position, vertices[0].size, vertices[0].normal_dir, vertices[0].color);
                }
            },
            Err(e) => {
