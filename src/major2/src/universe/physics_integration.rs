...
impl VoxelPhysicsGenerator {
    ...
    async fn generate_full_resolution(
        ...
        let dimensions = workspace.metadata.bounds.dimensions();
        let islands = self.find_collision_islands(&solid_mask, dimensions);
    ...
    async fn generate_lod_blocks(
        ...
        let dimensions = workspace.metadata.bounds.dimensions();
        let blocks_x = (dimensions.0 + block_size - 1) / block_size;
        let blocks_y = (dimensions.1 + block_size - 1) / block_size;
        let blocks_z = (dimensions.2 + block_size - 1) / block_size;
        
        for bz in 0..blocks_z {
            for by in 0..blocks_y {
                for bx in 0..blocks_x {
                    let block_offset = Vec3::new([
                        (bx * block_size) as f32 * workspace.metadata.bounds.voxel_size,
                        (by * block_size) as f32 * workspace.metadata.bounds.voxel_size,
                        (bz * block_size) as f32 * workspace.metadata.bounds.voxel_size,
                    ]);
                    ...
                    if self.block_contains_solid(workspace, (bx, by, bz), block_size) {
                        ...
                        let half_extents = Vec3::one() * (block_size as f32 * workspace.metadata.bounds.voxel_size * 0.5);
                        ...
                        colliders.push(VoxelPhysicsCollider {
                            ...
                            transform: Mat4f::from_translation(
                                workspace.metadata.bounds.min + block_offset + half_extents
                            ),
                            material_properties: MaterialProperties::default(),
                            lod_level,
                        });
        ...
        for z in 0..block_size {
            for y in 0..block_size {
                for x in 0..block_size {
                    ...
                    let dims_u32 = workspace.metadata.bounds.dimensions();
                    if wx < dims_u32.0 && 
                       wy < dims_u32.1 && 
                       wz < dims_u32.2 {
                        let idx = (wz * dims_u32.1 * dims_u32.0 +
                                  wy * dims_u32.0 + wx) as usize;
...
impl VoxelPhysicsUpdateSystem {
    ...
    pub async fn update_chunk_physics(
        ...
        let mut physics = self.physics_context.write();
...
impl VoxelCollisionQuery {
    ...
    pub fn raycast(&self, origin: Vec3<f32>, direction: Vec3<f32>, max_distance: f32) -> Option<RaycastHit> {
        let physics = self.physics_context.read();
