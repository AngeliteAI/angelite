...
void getFaceData(MeshRef meshRef, uint faceIndex, out uvec3 pos, out uvec2 size, out uint axis, out uint material) {
...
uvec3 posFromUVW(uint axis, uvec3 uvw) {
    ...
    if (axis % 2 == 1) {
        position[axisNormal] += 1;
    }
    
    return position;
}
// Function to generate vertex position based on UV coordinates for a specific face
vec3 generateVertexPosition(uvec3 pos, uvec2 size, uint axis, vec2 uv) {
