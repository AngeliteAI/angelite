...
@frozen public struct Surface {
  ...
  public class View: NSViewController {
    ...
    if let oldMetalView = metalView {
      ...
      // Create custom view
      let keyMetalView = KeyHandlingMTKView(frame: oldMetalView.frame, device: oldMetalView.device)
...
public override func mouseDown(with event: NSEvent) {
    ...
    public override func mouseMoved(with event: NSEvent) {
        ...
        self.inputHandler?.handleMouseMovement(deltaX: Float(event.deltaX), deltaY: Float(event.deltaY))
    }
