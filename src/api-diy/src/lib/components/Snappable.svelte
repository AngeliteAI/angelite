<script>
    ...
    import { mouseX, mouseY, virtualScale } from "$lib/store";
    ...
    function handleDragStart(customEvent) {
        let event = customEvent.detail; // Draggable component often wraps native event in detail
        if (!draggingEnabled) return;

        isDragging = true;
        
        startParent = event.startParent || element.parentElement;
        startIndex = event.startIndex !== undefined ? event.startIndex : Array.from(startParent.children).indexOf(element);
    ...
    function checkDropZones() {
        ...
        const draggedMarginBottom = parseFloat(draggedElStyle.marginBottom) || 0;
        ...
        if (parentEl) {
            ...
            for (const siblingEl of children) {
                ...
                const siblingMarginTop = parseFloat(siblingStyle.marginTop) || 0;
                const siblingMarginBottom = parseFloat(siblingStyle.marginBottom) || 0;
                const siblingMarginLeft = parseFloat(siblingStyle.marginLeft) || 0;
                const siblingMarginRight = parseFloat(siblingStyle.marginRight) || 0;

                // Vertical gap (implies horizontal overlap with sibling)
                const horizontalOverlap = Math.max(0, Math.min(draggedElRect.right, siblingRect.right) - Math.max(draggedElRect.left, siblingRect.left));
                const minHorizontalOverlapForVerticalSnap = Math.min(draggedElRect.width, siblingRect.width) * 0.3;
                ...
                if (horizontalOverlap > minHorizontalOverlapForVerticalSnap) {
                    ...
                    const draggedBottomEdgeWithMargin = draggedElRect.bottom + draggedMarginBottom;
                    if (Math.abs(draggedBottomEdgeWithMargin - gapTopEdge) < snapThreshold && 
                        $mouseY < gapTopEdge && $mouseY > gapTopEdge - (siblingRect.height/2) - snapThreshold) { // Mouse is in upper half of conceptual gap
                         return { dropTarget: parentEl, referenceElement: siblingEl, dropAction: 'before' };
                    }

                    // Check for space BELOW sibling (draggedEl would be placed after siblingEl)
                    const gapBottomEdge = siblingRect.bottom + siblingMarginBottom;
                    const draggedTopEdgeWithMargin = draggedElRect.top - draggedMarginTop;
                    if (Math.abs(draggedTopEdgeWithMargin - gapBottomEdge) < snapThreshold &&
                        $mouseY > gapBottomEdge && $mouseY < gapBottomEdge + (siblingRect.height/2) + snapThreshold) { // Mouse is in lower half of conceptual gap
                        return { dropTarget: parentEl, referenceElement: siblingEl.nextSibling, dropAction: 'after' }; // insertAfter effectively
                    }
                }

                // Horizontal gap (implies vertical overlap with sibling)
                const verticalOverlap = Math.max(0, Math.min(draggedElRect.bottom, siblingRect.bottom) - Math.max(draggedElRect.top, siblingRect.top));
                const minVerticalOverlapForHorizontalSnap = Math.min(draggedElRect.height, siblingRect.height) * 0.3;
                ...
                if (verticalOverlap > minVerticalOverlapForHorizontalSnap) {
                    ...
                    const draggedRightEdgeWithMargin = draggedElRect.right + draggedMarginRight;
                    if (Math.abs(draggedRightEdgeWithMargin - gapLeftEdge) < snapThreshold &&
                        $mouseX < gapLeftEdge && $mouseX > gapLeftEdge - (siblingRect.width/2) - snapThreshold) { // Mouse in left half of conceptual gap
                        return { dropTarget: parentEl, referenceElement: siblingEl, dropAction: 'before' };
                    }
                    ...
                    const draggedLeftEdgeWithMargin = draggedElRect.left - draggedMarginLeft;
                    if (Math.abs(draggedLeftEdgeWithMargin - gapRightEdge) < snapThreshold &&
                        $mouseX > gapRightEdge && $mouseX < gapRightEdge + (siblingRect.width/2) + snapThreshold) { // Mouse in right half of conceptual gap
                        return { dropTarget: parentEl, referenceElement: siblingEl.nextSibling, dropAction: 'after' }; // insertAfter effectively
                    }
                }
            }
        ...
        for (let i = 0; i < potentialDropZones.length; i++) {
            ...
            if ($mouseX >= zoneRect.left - directHoverMargin && $mouseX <= zoneRect.right + directHoverMargin &&
                $mouseY >= zoneRect.top - directHoverMargin && $mouseY <= zoneRect.bottom + directHoverMargin) {
                ...
                if (isContainerLike) {
                    ...
                    if ($mouseY < zoneRect.top + thirdHeight) {
                         return { dropTarget: zoneInfo.domElement, referenceElement: zoneInfo.domElement.firstChild, dropAction: 'prepend' }; // Prepend
                    } else if ($mouseY > zoneRect.bottom - thirdHeight) {
                         return { dropTarget: zoneInfo.domElement, referenceElement: null, dropAction: 'append' }; // Append
                    } else {
                        // Middle part of container, default to append or a specific 'inside' action
                         return { dropTarget: zoneInfo.domElement, referenceElement: null, dropAction: 'append' }; 
                    }
                } else {
    ...
    function handleDragMove(customEvent) {
        ...
        const { dropTarget, dropAction, referenceElement: newRef } = checkDropZones();
        ...
        if (dropTarget && dropAction) {
            ...
            dropPreviewElement.style = window.getComputedStyle(contentElement.firstChild);
            ...
            let marginBottom = parseFloat(margins[0]) || parseFloat(margins[2]) || 0;
    ...
    function handleDragEnd(customEvent) {
        ...
        isDragging = false;
        let draggableEventDetail = customEvent.detail || {};
            element.offsetHeight; // Force reflow after DOM move let actualFinalRect = element.getBoundingClientRect();
            console.log("[Snappable handleDragEnd] Actual element BoundingRect AFTER DOM move:", actualFinalRect);
    ...
    function moveElement(elementToMove, actionTargetElement, actionName) {
        ...
        if (elementToMove === element && previewMode) {
            console.log("[Snappable moveElement] Bailing out: previewMode is true for main element.");
            return;
        }

        if (!actionTargetElement || !elementToMove) {
            console.warn("[Snappable moveElement] Bailing out: Missing elementToMove or actionTargetElement.", 
            ...
            switch (actionName) {
                case 'before': // actionTargetElement is the SIBLING to insert before
                    if (actionTargetElement.parentNode) {
                        actionTargetElement.parentNode.insertBefore(elementToMove, actionTargetElement);
                    } else {
                        console.error("moveElement ('before'): actionTargetElement has no parentNode.", actionTargetElement);
                    }
                    break;
                case 'after': // actionTargetElement is the SIBLING to insert after
                    if (actionTargetElement.parentNode) {
                        actionTargetElement.parentNode.insertBefore(elementToMove, actionTargetElement.nextSibling);
                    } else {
                        console.error("moveElement ('after'): actionTargetElement has no parentNode.", actionTargetElement);
                    }
                    break;
                ...
                case 'prepend': // actionTargetElement is the CONTAINER
                    actionTargetElement.insertBefore(elementToMove, actionTargetElement.firstChild);
                    break;
                default:
                    console.warn("moveElement: Unknown actionName:", actionName, "Falling back to append if possible.");
                    if (actionTargetElement.appendChild) { // Fallback: attempt to append to actionTargetElement
                        actionTargetElement.appendChild(elementToMove);
                    }
                    break;
            }
...
<div 
    ...
    class="snappable {isSnapped ? 'snappable-snapped' : ''} {isDragging ? 'snappable-dragging' : ''}"
    data-node-id={id}
>
    <Draggable
    ...
    >
        <div class="snappable-content" bind:this={contentElement} bind:clientWidth bind:clientHeight>
            {@render children()}
        </div>
    </Draggable>
</div>
