...
<div
    ...
    id="viewport"
    style="--scale-x: {backgroundScale}%; --scale-y: {(backgroundScale * w) /
        h}%; --origin-x: {origin.x + w / 2}px; --origin-y: {origin.y}px;"
    bind:clientWidth={w}
    bind:clientHeight={h}
    role="region"
    onmousemove={(e) => {
    ...
    onwheel={(e) => {
        ...
        const c_mouse_x =
            (zoomCalcMouseX - px_drag / S_prime - ox_old) / S_prime;
        const c_mouse_y =
            (zoomCalcMouseY - py_drag / S_prime - oy_old) / S_prime;
...
>
    <Draggable bind:this={draggableComponent} bind:position screenspace={true}>
        <div
            id="camera"
            class="absolute"
            style="width: {w}px; height: {h}px;
            transform: translate({-position.x}px, {-position.y}px); left: 0;
