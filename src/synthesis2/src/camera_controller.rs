...
impl CameraController {
    pub fn new() -> Self {
        // Position camera at (20, 20, 20) looking at origin
        // In our coordinate system: X=East, Y=North, Z=Up
        Self::new_look_at(Vec3f::xyz(20.0, 20.0, 20.0), Vec3f::ZERO, Vec3f::Z)
    }
    ...
    pub fn update(&mut self, engine: &dyn Engine, delta_time: f32) {
        ...
        // Button states
        let _select_pressed = unsafe { engine.input_binding_data(Binding::Select).activate };
        let _escape_pressed = unsafe { engine.input_binding_data(Binding::Escape).activate };
        ...
        if let Some(physx) = engine.physx() {
            if let Some(body) = self.physics_body {
                ...
                // Step physics simulation with safety check
                if delta_time > 0.0 && delta_time < 1.0 {
                    physx.step(delta_time);
                } else {
                    println!("WARNING: Skipping physics step with invalid delta_time: {}", delta_time);
                }
                
                // Read back state from physics
                let position_before_physics = self.position;
                let new_position = physx.rigidbody_get_position(body);
                let new_rotation = physx.rigidbody_get_orientation(body);
                
                // Sanity check the physics results
                if new_position[0].is_finite() && new_position[1].is_finite() && new_position[2].is_finite() {
                    self.position = new_position;
                } else {
                    println!("ERROR: Physics returned non-finite position! Keeping old position.");
                }
                
                if new_rotation.1.is_finite() && new_rotation.0[0].is_finite() && 
                   new_rotation.0[1].is_finite() && new_rotation.0[2].is_finite() {
                    self.rotation = new_rotation;
                } else {
                    println!("ERROR: Physics returned non-finite rotation! Keeping old rotation.");
                }
