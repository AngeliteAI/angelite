...
impl VoxelChunkRenderer {
    ...
    pub fn update_chunk(
        ...
        let mesh = if let Some(chunk_mesh) = self.chunk_meshes.get(&chunk_id) {
            println!("Updating existing mesh for chunk {:?}", chunk_id);
            chunk_mesh.mesh
        } else {
            println!("Creating new mesh for chunk {:?}", chunk_id);
            let new_mesh = self.gfx.mesh_create();
            self.gfx.batch_add_mesh(self.batch, new_mesh);
            println!("Added mesh to batch for chunk {:?}", chunk_id);
        ...
        println!("Chunk {:?} mesh updated with {} vertices, total chunks in renderer: {}", 
                 chunk_id, vertices.len(), self.chunk_meshes.len());
    }
