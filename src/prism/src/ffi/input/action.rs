...
unsafe extern "C" {
    ...
    pub fn addMouseButtonBinding(
        action: *mut c_void,
        button: MouseButton,
        action_type: ButtonAction,
    ) -> bool;
    pub fn addGamepadButtonBinding(
        action: *mut c_void,
...
impl ActionManager {
    ...
    pub fn create_action(&self, name: &str) -> Result<ActionId, &'static str> {
        let id = unsafe { createAction(self.ptr, name.as_ptr() as *const c_char, name.len()) };
        Ok(id)
    }
...
impl InputAction {
    ...
    pub fn add_mouse_button_binding(
        &self,
        button: MouseButton,
        action_type: ButtonAction,
    ) -> Result<(), &'static str> {
    ...
    pub fn add_gamepad_button_binding(
        &self,
        button: GamepadButton,
        action_type: ButtonAction,
    ) -> Result<(), &'static str> {
