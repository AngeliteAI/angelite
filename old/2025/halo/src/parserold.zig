            ...
            .Operator => |op| {
                ...
                switch (op) {
                    ...
                    .@"&", .@"|", .@"^", .@"+", .@"-", .@"*", .@"/", .@"%", .@"==", .@"!=", .@"<", .@">", .@"<=", .@">=", .@"<<", .@">>" => {
                        ...
                        //
                        if (op == .@"*" and evalStack.items.len < 2) {
                            ...
                            //     continue;
                            // }
                            const variable = evalStack.pop().?;
                            if (variable.data == ExpressionData.Variable) {
                                //return unit to the caller

                                //now make a pointer expression
                                const deref = try state.allocator.create(PointerDeref);
                                deref.* = PointerDeref { .ptr = variable };
                                const pointer = try state.allocator.create(Expression);
                                pointer.* = Expression.init(ExpressionData{ .PointerDeref = deref });
                                try evalStack.append(pointer);

                                return pointer;
                            } else {
                                //return unit
                                const unit = try state.allocator.create(Expression);
                                unit.* = Expression.init(ExpressionData{ .Literal = .unit });
                                try evalStack.append(unit);
